            ...
            {['MISTRAL', 'LLAMA', 'QWEN'].map(m => (
                <button 
                    ...
                    className={`px-2 py-1 rounded text-[9px] font-bold transition-colors border ${
                        activeModel === m 
                        ? 'bg-diamond-500 text-whale-900 border-diamond-500' 
                        : 'bg-whale-900 text-slate-500 border-whale-700 hover:text-white'
                    }`}
                >
                    {m}
                </button>
      ...
      <div className="flex-1 bg-whale-950 p-4 overflow-y-auto scrollbar-thin scrollbar-thumb-whale-700" ref={scrollRef}>
          ...
          {isThinking && (
              <div className="flex items-center gap-2 text-trenchPurple-500/50 text-xs font-mono animate-pulse pl-2">
                  <Brain size={12} />
                  <span>Processing context vectors ({activeModel})...</span>
              </div>
          )}
      </div>
