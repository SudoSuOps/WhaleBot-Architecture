    ...
    <div className="h-[500px] w-full bg-whale-800 border border-whale-700 rounded-xl p-4 shadow-lg shadow-black/20 flex flex-col">
      <div className="flex flex-col xl:flex-row justify-between items-start xl:items-center mb-4 gap-4">
        <div className="flex flex-col gap-2 w-full xl:w-auto">
            <div className="flex items-center gap-2"><span className="text-[10px] text-slate-500 font-bold uppercase tracking-wider w-12">Majors</span><div className="flex bg-whale-900 rounded-lg p-1 border border-whale-700">{majors.map(asset => (<button key={asset} onClick={() => onAssetChange(asset)} className={`px-3 py-1 rounded-md text-xs font-bold transition-all ${selectedAsset === asset ? 'bg-diamond-500 text-whale-900 shadow-[0_0_10px_rgba(100,255,218,0.3)]' : 'text-slate-400 hover:text-slate-200'}`}>{asset}</button>))}</div></div>
            <div className="flex items-center gap-2"><span className="text-[10px] text-trenchPurple-500 font-bold uppercase tracking-wider w-12">Trench</span><div className="flex bg-whale-900 rounded-lg p-1 border border-whale-700/50">{trenches.map(asset => (<button key={asset} onClick={() => onAssetChange(asset)} className={`px-3 py-1 rounded-md text-xs font-bold transition-all ${selectedAsset === asset ? 'bg-trenchPurple-500 text-white shadow-[0_0_10px_rgba(168,85,247,0.3)]' : 'text-slate-500 hover:text-trenchPurple-400'}`}>{asset}</button>))}</div></div>
        </div>
        <div className="flex items-center justify-between w-full xl:w-auto gap-4">
             <div className="flex bg-whale-900 rounded-lg p-1 border border-whale-700">{['15m', '1h', '4h'].map((tf) => (<button key={tf} onClick={() => onTimeframeChange(tf as Timeframe)} className={`px-3 py-1 rounded text-[10px] font-bold transition-all ${timeframe === tf ? 'bg-whale-700 text-white' : 'text-slate-500 hover:text-slate-300'}`}>{tf}</button>))}</div>
             <span className="text-3xl font-mono text-diamond-500 animate-in fade-in duration-300 whitespace-nowrap">${currentPrice.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 4 })}</span>
            <div className="flex items-center gap-2"><div className="flex bg-whale-900 rounded-lg p-1 border border-whale-700"><button onClick={() => onDataSourceChange('KRAKEN')} className={`px-3 py-1 rounded text-[10px] font-bold transition-all ${dataSource === 'KRAKEN' ? 'bg-trenchGold-500 text-whale-900' : 'text-slate-500 hover:text-slate-300'}`}>KRAKEN</button><button onClick={() => onDataSourceChange('HYPERLIQUID')} className={`px-3 py-1 rounded text-[10px] font-bold transition-all ${dataSource === 'HYPERLIQUID' ? 'bg-trenchPurple-500 text-white' : 'text-slate-500 hover:text-slate-300'}`}>HYPERLIQUID</button></div><div className="flex items-center gap-2 px-2 py-1 bg-whale-900 rounded text-xs text-green-400 font-mono border border-whale-700/50"><div className="w-2 h-2 rounded-full bg-green-500 animate-pulse"></div>LIVE</div></div>
        </div>
      </div>
