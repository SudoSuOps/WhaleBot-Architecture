...
import { Position, WhaleTransaction, MemeResponse } from '../types';
...
const MemeReactor: React.FC<MemeReactorProps> = ({ positions = [], whaleFeed = [] }) => {
    ...
    const handleGenerate = async (customTopic?: string) => {
        ...
        const data = await generateMemeJSON(ctx as any);
        setMemeData(data);
        setIsGenerating(false);
    };

    useEffect(() => {
        if (memeData && canvasRef.current) {
            renderMemeToCanvas(memeData, canvasRef.current);
        }
    }, [memeData]);
    ...
    const handleTwitterShare = () => {
        if (!memeData) return;
        const tweetText = `${memeData.headline}\n\n${memeData.subline}\n\nGenerated by @WhalePerp AI 🐳 ${memeData.hashtags.join(' ')}`;
                ...
                <div className="bg-whale-800 border border-trenchGold-500/30 rounded-xl p-8 relative overflow-hidden">
                    ...
                    <div className="flex justify-between items-center mb-8 relative z-10">
                        <div className="flex items-center gap-3">
                            <div className="p-2 bg-whale-900 rounded-lg border border-trenchGold-500/50 shadow-[0_0_15px_rgba(255,215,0,0.2)]"><Sparkles className="text-trenchGold-500" size={24} /></div>
                            <div><h1 className="text-3xl font-black text-white tracking-tight leading-none">MEME REACTOR</h1><p className="text-xs text-trenchPurple-400 font-mono tracking-widest">v1.4 // NUCLEAR CONTEXT</p></div>
                        </div>
                    </div>
                    ...
                    <div className="space-y-6 relative z-10">
                        ...
                        <div className="flex gap-2"><input type="text" value={topic} onChange={e => setTopic(e.target.value)} className="flex-1 bg-whale-900 border border-whale-700 rounded-lg px-4 py-3 text-white focus:border-trenchGold-500 outline-none font-mono text-sm placeholder:text-slate-600" placeholder="Enter topic (e.g. ETH, TRUMP, JEETS)..." /><button onClick={() => handleGenerate()} disabled={isGenerating} className="bg-trenchGold-500 hover:bg-trenchGold-400 text-whale-900 font-bold px-6 py-3 rounded-lg transition-transform active:scale-95 flex items-center gap-2 shadow-[0_0_20px_rgba(255,215,0,0.3)]">{isGenerating ? <RefreshCw className="animate-spin" size={20}/> : <Zap size={20} fill="currentColor"/>} MINT MEME</button></div>
                    </div>
                </div>

                <div className={`bg-whale-900 border border-whale-700 rounded-xl overflow-hidden shadow-2xl transition-all duration-500 ${memeData ? 'opacity-100 scale-100' : 'opacity-50 scale-95'}`}>
                    <div className="relative bg-black aspect-video flex items-center justify-center">
                        {!memeData && <p className="text-slate-600 font-mono text-xs animate-pulse">AWAITING INPUT DATA...</p>}
                        <canvas ref={canvasRef} width={800} height={450} className={`w-full h-full ${!memeData && 'hidden'}`} />
                    </div>
                    <div className="bg-whale-800 p-4 flex justify-between items-center border-t border-whale-700">
                        <div className="flex gap-2"><button onClick={handleDownload} disabled={!memeData} className="p-2 bg-whale-700 hover:bg-whale-600 rounded text-slate-300 transition-colors flex items-center gap-2 text-xs font-bold px-4 border border-whale-600 disabled:opacity-50"><Download size={14}/> SAVE PNG</button></div>
                        <button onClick={handleTwitterShare} disabled={!memeData} className="flex items-center gap-2 bg-[#1DA1F2] hover:bg-[#1a91da] text-white px-5 py-2 rounded font-bold text-xs transition-transform active:scale-95 disabled:opacity-50 shadow-lg"><Twitter size={14} fill="currentColor"/> POST TO X</button>
                    </div>
                </div>
            </div>

            <div className="lg:col-span-4 bg-whale-900 border border-whale-700 rounded-xl flex flex-col h-[600px]"><div className="p-4 border-b border-whale-800 bg-whale-800/50"><h3 className="font-bold text-white flex items-center gap-2"><ImageIcon size={18} className="text-trenchPurple-500"/> LIVE FEED</h3></div><div className="flex-1 p-8 flex items-center justify-center text-slate-600 italic text-sm">Community Feed Coming Soon...</div></div>
        </div>
    );
};
