...
// --- REAL COMPUTE CONNECTION ---
// In production, your local rig pushes data to a KV store or Worker Endpoint
// The frontend fetches that "True Truth" here.
const fetchRigProbability = async (): Promise<number | null> => {
    try {
        // Example endpoint you would build: https://whale-api.yourdomain.com/quant/probability
        // const res = await fetch('https://your-rig-endpoint.com/probability');
        // const data = await res.json();
        // return data.score;
        return null; // Currently disconnected
    } catch (e) { return null; }
};
...
export const calculateQuantMetrics = (): QuantMetrics => {
    ...
    // 1. Attempt to get Real Rig Data (Future Integration)
    // const rigScore = await fetchRigProbability();
    // if (rigScore !== null) totalScore = rigScore;
    ...
    const signals: QuantSignal[] = WHALE_CONFIG.quant.signals.map(sig => {
        ...
        return { id: sig.id, name: sig.id.replace(/_/g, ' ').toUpperCase(), description: sig.description, weight: sig.weight, score: rawScore, contribution };
    });
    ...
    return { totalScore: smoothedScore, bias, signals: signals.sort((a,b) => b.contribution - a.contribution) };
};
