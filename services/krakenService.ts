...
// Mapping Kraken Futures symbols to our internal Asset IDs
// PI_ = Perpetual Inverse (Coin Margined)
// PF_ = Perpetual Linear (USD Margined)
const ASSET_MAPPING: Record<string, string> = {
  'PI_XBTUSD': 'BTC', 'PF_XBTUSD': 'BTC',
  'PI_ETHUSD': 'ETH', 'PF_ETHUSD': 'ETH',
  'PI_SOLUSD': 'SOL', 'PF_SOLUSD': 'SOL',
  'PI_XRPUSD': 'XRP', 'PF_XRPUSD': 'XRP',
  // Trench Assets (Linear Only usually)
  'PF_TAOUSD': 'TAO',
  'PF_ASTRUSD': 'ASTR',
  'PF_DOGEUSD': 'DOGE',
  'PF_BNBUSD': 'BNB'
  ...
  try {
    ...
    socket.onmessage = (event) => {
      ...
      if (data.feed === 'ticker' && data.product_id) {
        ...
        if (asset && data.last) {
           onPriceUpdate(asset, data.last, data.volume || 0);
        }
      }
    };
    ...
    console.error("Failed to connect to Kraken:", e);
  }
