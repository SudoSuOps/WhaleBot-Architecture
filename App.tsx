...
import MemeReactor from './components/MemeReactor';
...
const App: React.FC = () => {
  const loadState = <T,>(key: string, def: T): T => { try { const saved = localStorage.getItem(key); return saved ? JSON.parse(saved) : def; } catch (e) { return def; } };
  ...
  const handleTabChange = (tab: string) => { if ((tab === 'whalebot' || tab === 'warroom' || tab === 'meme-reactor') && (!userProfile || userProfile.tier === 'FREE')) { setShowCheckout(true); return; } setActiveTab(tab); };
